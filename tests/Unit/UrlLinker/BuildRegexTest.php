            ...
            'empty options' => [
                ...
                'expected' => <<<PRCE
                    #\b
                        (https?://)?
                        (?:
                            ([^]\\\\\\x00-\\x20\"(),:-<>[\\x7f-\\xff]{1,64})
                            (:[^]\\\\\\x00-\\x20\"(),:-<>[\\x7f-\\xff]{1,64})?
                        @)?
                        ((?:[-a-zA-Z0-9\\x7f-\\xff]{1,63}\.)+[a-zA-Z\\x7f-\\xff][-a-zA-Z0-9\\x7f-\\xff]{1,62}|(?:[1-9]\d{0,2}\.|0\.){3}(?:[1-9]\d{0,2}|0))
                        ((:[0-9]{1,5})?(/[!$-/0-9:;=@_':;!a-zA-Z\\x7f-\\xff]*?)?(\?[!$-/0-9:;=@_':;!a-zA-Z\\x7f-\\xff]+?)?(\#[!$-/0-9?:;=@_':;!a-zA-Z\\x7f-\\xff]+?)?)
                        (?=[)'?.!,;:]*
                            ([^-_\#$+.!*%'(),;/?:@=&a-zA-Z0-9\x7f-\xff]|$)
                        )
                    #x
